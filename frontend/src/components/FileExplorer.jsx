...
// ✅ Attach Firebase JWT to all requests
...
export default function FileExplorer() {
  ...
  const fetchFiles = async () => {
    try {
      const res = await API.get(`/files?trash=${showTrash}&folder=${currentFolder || ""}`);
      setFiles(res.data || []);
    } catch (err) {
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {files.map((file) => (
          <div
            key={file._id}
            className="border rounded p-3 flex flex-col shadow bg-white"
          >
            {/* File Icon + Name */}
